...
class RMSNorm(nn.Module):
    ...
    def _norm(self, x):
        # 计算RMSNorm的核心部分
        # x.pow(2).mean(-1, keepdim = True) 计算输入了x的平方的均值
...
def repeat_kv(x: torch.Tensor, n_rep: int) -> torch.Tensor:
    ...
    return(
        ...
        .expand(bs, slen, n_kv_heads, n_rep, head_dim) # 将新添加的维度拓展到n_rep大小，实现重复的效果
        .reshape(bs, slen, n_kv_heads * n_rep, head_dim) # 重新塑性，合并键/值对头的数量和重复次数的维度
    )
...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, freqs_cos: torch.Tensor, freqs_sin: torch.Tensor):
        ...
        if self.flash:
            # 使用Flash Attention
            output = nn.functional.scaled_dot_product_attention(xq, xk, xv, attn_mask=None, dropout_p=self.dropout if self.training else 0.0, is_causal=True)
        else:
            # 使用手动实现的注意力机制
...
class DecoderLayer(nn.Module):
    def __init__(self, layer_id: int, args: ModelConfig):
        ...
        # 定义输入维度
        self.dim = args.dim
        # 定义每个头的维度，等于输入的维度除以头数
        self.head_dim = args.dim // args.n_heads
        # 定义Attention对象，用于进行多头注意力计算
        self.attention = Attention(args)
        # 定义MLP对象，用于进行前反馈神经网络计算
    ...
    def forward(self, x, freqs_cos, freqs_sin):
        ...
        h = x + self.attention.forward(self.attention_norm(x), freqs_cos=freqs_cos, freqs_sin=freqs_sin)
...
# decoderlayer = DecoderLayer(0, config)
